...
func main() {
	...
	runtime.ReadMemStats(&standardMemStats)
	standardEndAlloc := standardMemStats.Alloc
	defer fmt.Printf("Standard version of SHA3-512 hashes text on average using %v bytes\n",
		(standardEndAlloc-standardStartAlloc)/rounds)
	var implMemStats runtime.MemStats
	runtime.ReadMemStats(&implMemStats)
	implStartAlloc := implMemStats.Alloc
	for i := 0; i < rounds; i++ {
		_ = implSha3.Sum512(data)
	}
	runtime.ReadMemStats(&implMemStats)
	implEndAlloc := implMemStats.Alloc
	defer fmt.Printf("Implemented version of SHA3-512 hashes text on average using %v bytes\n",
		(implEndAlloc-implStartAlloc)/rounds)
}
